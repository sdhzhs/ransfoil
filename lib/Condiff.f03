...
real(8) Xi,fnu1,fnu2,Sv,rm,gm,Ret,Dwplus,phi1,F1,betaistar,Fmt,alphaf,betai
  ...
  DO i=2,Ic-1
   ...
   if(j==1.and.(i>=Ib1.and.i<=Ib2)) then
    ...
    if((Turmod=='sa'.and.Walltreat=='lr').or.(Turmod=='sst'.and.Walltreat=='lr').or.Turmod=='lam'.or.Turmod=='inv') then
     ...
     if(scalar=='U') then
      aP(i,j)=aW(i,j)+aE(i,j)+aS(i,j)+aN(i,j)+DF(i,j)+rho(i,j)*ustar(i)*DR(i)/Uplus(i)*(Yga(i,j)/da(i,j))**2
     else if(scalar=='V') then
      aP(i,j)=aW(i,j)+aE(i,j)+aS(i,j)+aN(i,j)+DF(i,j)+rho(i,j)*ustar(i)*DR(i)/Uplus(i)*(Xga(i,j)/da(i,j))**2
     else if(scalar=='T') then
      if(minval(Tplus)<0) then
       aP(i,j)=aW(i,j)+aE(i,j)+aS(i,j)+aN(i,j)+DF(i,j)
      else
       aP(i,j)=aW(i,j)+aE(i,j)+aS(i,j)+aN(i,j)+DF(i,j)+rho(i,j)*ustar(i)*DR(i)/Tplus(i)
      end if
     else if(scalar=='Tn') then
      aP(i,j)=aW(i,j)+aE(i,j)+aS(i,j)+aN(i,j)+DF(i,j)+2*Ds(i,j)
     else if(Turmod=='sst'.and.scalar=='Tk') then
      aP(i,j)=aW(i,j)+aE(i,j)+aS(i,j)+aN(i,j)+DF(i,j)+rho(i,j)*ustar(i)**3*Uplus(i)*Jg(i,j)*dx*dy/(Tk(i,j)*Yp(i))
  ...
  if(scalar=='U') then
   ...
   if(j==1.and.(i>=Ib1.and.i<=Ib2)) then
    if((Turmod=='sa'.and.Walltreat=='lr').or.(Turmod=='sst'.and.Walltreat=='lr').or.Turmod=='lam'.or.Turmod=='inv') then
     ...
     if(Proctrl=='com') then
      if(minval(Tplus)<0) then
       b(i,j)=Jg(i,j)*(U(i,j)*Px(i,j)+V(i,j)*Py(i,j))*dx*dy/ca+rho(i,j)*ustar(i)*(Tf-T(i,j))*DR(i)/Tplus(i)
      else
       b(i,j)=Jg(i,j)*(U(i,j)*Px(i,j)+V(i,j)*Py(i,j))*dx*dy/ca+rho(i,j)*ustar(i)*Tf*DR(i)/Tplus(i)
      end if
     else if(Proctrl=='incom') then
      if(minval(Tplus)<0) then
       b(i,j)=rho(i,j)*ustar(i)*(Tf-T(i,j))*DR(i)/Tplus(i)
      else
