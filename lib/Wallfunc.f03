...
DO i=Ib1,Ib2
  if(Turmod=='sst') then
  ustar(i)=((miu(i,1)*sqrt(U(i,1)**2+V(i,1)**2)/(rou(i,1)*Dyp(i)))**2+(alpha1*Tk(i,1))**2)**0.25
  else if(Turmod=='sa') then
  ustar(i)=((miu(i,1)*sqrt(U(i,1)**2+V(i,1)**2)/(rou(i,1)*Dyp(i)))**2+(miut(i,1)/(rou(i,1)*kapa*Dyp(i)))**4)**0.25
  else if(Turmod=='ke') then
  ustar(i)=Cu**0.25*Tk(i,1)**0.5
  end if
  ksplus(i)=rou(i,1)*ks(i)*ustar(i)/miu(i,1)
  if(Turmod=='sa'.or.Turmod=='sst') then
  Yplus(i)=rou(i,1)*Dyp(i)*ustar(i)/miu(i,1)
  if(Walltreat=='wf'.and.ks(i)>1d-10) then
  Yplus(i)=max(Yplus(i),ksplus(i)/2,2.5)
  end if
  else if(Turmod=='ke') then
  Ystar(i)=rou(i,1)*Dyp(i)*ustar(i)/miu(i,1)
  Ystar(i)=max(Ystar(i),ksplus(i)/2,11.225)
  end if
...
if((Turmod=='sa'.or.Turmod=='sst').and.Walltreat=='wf') then
 ...
 DO i=Ib1,Ib2
  ...
  exp(1./lamda(i))*0.5*rou(i,1)*ustar(i)*(Prt*(U(i,1)**2+V(i,1)**2)+(Pr(i,1)-Prt)*(11.8*ustar(i))**2))/Tplus(i)
  Tplusl(i)=Pr(i,1)*Uplusl(i)+0.5*rou(i,1)*ustar(i)*Pr(i,1)*(U(i,1)**2+V(i,1)**2)/Q(i)
  Tplust(i)=Prt*(Uplust(i)+Prough(i))+0.5*rou(i,1)*ustar(i)*(Prt*(U(i,1)**2+V(i,1)**2)+(Pr(i,1)-Prt)*(11.8*ustar(i))**2)/Q(i)
  Tplus(i)=exp(lamda(i))*Tplusl(i)+exp(1./lamda(i))*Tplust(i)
  else
  Tplusl(i)=Pr(i,1)*Uplusl(i)
  Tplust(i)=Prt*(Uplust(i)+Prough(i))
...
DO i=Ib1,Ib2
 ...
 (Pr(i,1)-Prt)*(11.8*ustar(i))**2))/Tplus(i)
 Tplus(i)=Prt*(Uplus(i)+Prough(i))+0.5*rou(i,1)*ustar(i)*(Prt*(U(i,1)**2+V(i,1)**2)+(Pr(i,1)-Prt)*(11.8*ustar(i))**2)/Q(i)
 else
 Tplus(i)=Prt*(Uplus(i)+Prough(i))
 Q(i)=ca*rou(i,1)*ustar(i)*(Tf-T(i,1))/Tplus(i)
 end if
...
lamda=-0.01*Yplus**4/(1+5.*Yplus)
Twplusl=min(Twplusl,6./(betai*Yplus**2))
