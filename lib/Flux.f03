...
DO j=1,Jc-1
 DO i=Is,Ie+1
  if(isCoup) then
   if(i==1) then
    Uf=interpl(Up(Ic,j),Up(i,j),5d-1)
    Vf=interpl(Vp(Ic,j),Vp(i,j),5d-1)
   else if(Is>1.and.i==2) then
    Uf=interpl(U(i-1,j),Up(i,j),5d-1)
    Vf=interpl(V(i-1,j),Vp(i,j),5d-1)
   else if(Is>1.and.i==Ic) then
    Uf=interpl(Up(i-1,j),U(i,j),5d-1)
    Vf=interpl(Vp(i-1,j),V(i,j),5d-1)
   else if(i==Ip) then
    Uf=interpl(Up(i-1,j),Up(1,j),5d-1)
    Vf=interpl(Vp(i-1,j),Vp(1,j),5d-1)
   else
    Uf=interpl(Up(i-1,j),Up(i,j),5d-1)
    Vf=interpl(Vp(i-1,j),Vp(i,j),5d-1)
   end if
  else
   if(i==1) then
 ...
 DO i=Is,Ie
  if(isCoup) then
   if(j==1.and.(i>Ib2.or.i<Ib1)) then
    Uf=interpl(Up(Ic+1-i,j),Up(i,j),5d-1)
    Vf=interpl(Vp(Ic+1-i,j),Vp(i,j),5d-1)
   else if(j==1) then
    Uf=0
    Vf=0
   else if(j==Jc) then
    Uf=interpl(Up(i,j-1),U(i,j),5d-1)
    Vf=interpl(Vp(i,j-1),V(i,j),5d-1)
   else
    Uf=interpl(Up(i,j-1),Up(i,j),5d-1)
    Vf=interpl(Vp(i,j-1),Vp(i,j),5d-1)
   end if
  else
   if(j==1.and.(i>Ib2.or.i<Ib1)) then
