...
dsp,dsm,dsp0,dsm0,Xgk,Ygk,Xga,Yga,Det,Xgkm,Ygkm,Xgam,Ygam,Detm
real(8) V(Ip),Xg0(Ip),Yg0(Ip),V0(Ip),Xg00(Ip),Yg00(Ip),epsi(Ip),stab1(Ip),stab2(Ip),N(Ip),d(Ip),d0(Ip)
...
DO j=2,Jp
 ...
 a=1/(1-cosa**2)
 else if(acos(cosa)>Pi/2.and.acos(cosa)<=Pi) then
 a=1
 end if
 epsi(i)=epsic*N(i)*Se*df*a
 stab1(i)=epsi(i)*((Xg0(i+1)+Xg0(i-1)-2*Xg0(i))-(Xg00(i+1)+Xg00(i-1)-2*Xg00(i)))
 stab2(i)=epsi(i)*((Yg0(i+1)+Yg0(i-1)-2*Yg0(i))-(Yg00(i+1)+Yg00(i-1)-2*Yg00(i)))
 end DO
 end if
 DO i=2,Ip-1
 Xgk=(Xg0(i+1)-Xg0(i-1))/2
 ...
 axW1=(1+theta)*(Xgk*Xga-Ygk*Yga)/Det/2+2*epsi(i)
 axE1=-(1+theta)*(Xgk*Xga-Ygk*Yga)/Det/2+2*epsi(i)
 axP1=1+4*epsi(i)
 ayP1=0
 b1=-Ygk*V(i)/Det-stab1(i)
 ayW2=(1+theta)*(Ygk*Yga-Xgk*Xga)/Det/2+2*epsi(i)
 ayE2=-(1+theta)*(Ygk*Yga-Xgk*Xga)/Det/2+2*epsi(i)
 axW2=(1+theta)*(Xgk*Yga+Ygk*Xga)/Det/2
 axE2=-(1+theta)*(Xgk*Yga+Ygk*Xga)/Det/2
 axP2=0
 ayP2=1+4*epsi(i)
 b2=Xgk*V(i)/Det-stab2(i)
 Ap(1,1,i)=axP1
 Aw(1,1,i)=axW1
 Ae(1,1,i)=axE1
 Ap(1,2,i)=ayP1
